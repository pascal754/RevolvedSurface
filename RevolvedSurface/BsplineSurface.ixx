...
public:
    ...
    void surfacePointW(double u, double v, Point3D& S);
    void addVector(const std::vector<Point3D>& vPt);
    void makeKnots();
    void assignControlPoints(const std::vector<std::vector<Point3D>>& cp);
    void assignWeight(const std::vector<std::vector<double>>& w);
    void assignUKnots(const std::vector<double>& kv);
    void assignVKnots(const std::vector<double>& kv);
private:
    ...
    std::vector<std::vector<double>> weight; // n + 1 by m + 1 matrix
